...
Claude Code 不是聊天机器人，而是一个六层架构的代理系统。只强化其中一层，系统就会失衡。
...
| 类型 | 用途 | 特点 | 示例 |
|------|------|------|------|
| **Tool/MCP** | 给 Claude 新动作能力 | 运行时调用 | GitHub API、文件操作 |
| **Skill** | 给它一套工作方法 | 上下文指导 | PR review 流程、debug 步骤 |
| **Subagent** | 隔离执行环境 | 独立上下文 | 探索代码、并行研究 |
| **Hook** | 强制约束和审计 | 确定性脚本 | pre-commit 检查、权限阻断 |
| **Plugin** | 跨项目分发 | 打包复用 | 团队共享配置 |
...
**典型场景**：
```bash
# 主会话：明确目标，协调 Subagents
# Subagent A：探索 API 实现
# Subagent B：探索测试覆盖
# Subagent C：探索文档
# 主会话：汇总结果，做决策
```
